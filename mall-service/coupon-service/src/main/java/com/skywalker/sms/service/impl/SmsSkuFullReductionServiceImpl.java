...
public class SmsSkuFullReductionServiceImpl implements SmsSkuFullReductionService {
    ...
    public void addSkuCouponInfo(SkuCouponTo skuCouponTo) {
        // 满减大于0才保存
        if (skuCouponTo.getFullPrice().compareTo(new BigDecimal("0")) > 0) {
...
//        smsSkuLadder.setPrice();
        ...
        skuCouponTo.getMemberPrice().stream()
                // 会员价格大于0的才保存
                .filter(ele -> ele.getPrice().compareTo(new BigDecimal("0")) > 0)
                .forEach(item -> {
