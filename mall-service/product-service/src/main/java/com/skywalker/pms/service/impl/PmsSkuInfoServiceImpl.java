...
public class PmsSkuInfoServiceImpl implements PmsSkuInfoService {
    ...
    public PageInfo<PmsSkuInfo> findPage(PmsSkuInfo pmsSkuInfo, int page, int size) {
        ...
        if (pmsSkuInfo != null) {
            List<Integer> priceRange = ((PmsSkuInfoVo) pmsSkuInfo).getPriceRange();
            // sku名称
            if (!StringUtils.isEmpty(pmsSkuInfo.getSkuName())) {
                criteria.andLike("skuName", "%" + pmsSkuInfo.getSkuName() + "%");
            }
            // 所属分类id
            ...
            // 价格
            if (!Collections.isEmpty(priceRange)) {
                criteria.andBetween("price", new BigDecimal(priceRange.get(0)), new BigDecimal(priceRange.get(1)));
            }
        }
        //执行搜索
        return new PageInfo<>(pmsSkuInfoMapper.selectByExample(example));
    }
